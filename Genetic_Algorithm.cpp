...
int selection(){
    if (random(1, 100) <= LUCK*100)
        return (random(POPULATION/2, POPULATION-1));
    else{
        int max = genome[(int)(POPULATION-2)/2].fit, r;
        while (true){
            r = random(0, POPULATION/2);
            if (genome[r].fit <= random(1, max))
              return r;
        }
    }
}
